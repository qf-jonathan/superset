...
export const dynamicPluginControlsReady =
  ...
  (dispatch: Dispatch, getState: () => RootState): void => {
    ...
    // TypeScript has trouble inferring the present type from UndoableExploreState
    // @ts-ignore
    const present: ExploreState = state.explore?.present;
    if (!present) return;

    // @ts-ignore
    const exploreState: ExploreState = {
      ...present,
      datasource: present.datasource || { type: 'table' },
    };
    const controlsState = getControlsState(
      exploreState as unknown as Parameters<typeof getControlsState>[0],
      present.form_data,

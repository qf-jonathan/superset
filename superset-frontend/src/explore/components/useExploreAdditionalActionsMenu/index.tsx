...
interface ExploreRootState {
  ...
  explore?: import('src/explore/types').ExplorePageState['explore'];
  common?: {
    conf?: {
...
export const useExploreAdditionalActionsMenu = (
  ...
  const chart = useSelector<ExploreRootState, ChartState | undefined>(state =>
    state.explore?.present
      ? state.charts?.[getChartKey(state.explore.present)]
      : undefined,
  );
  const streamingThreshold = useSelector<ExploreRootState, number>(
    state =>
